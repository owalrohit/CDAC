...
public class CandidateListServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try (PrintWriter pw = response.getWriter()) {
			...
			HttpSession hs = request.getSession();//servlet req to WC for to either create n return NEW HttpSession or return the existing one ! 
			System.out.println("Session ID " + hs.getId());// same session id , provided cookies are accepted by the client
			System.out.println("Session is new from candidate list page " + hs.isNew());// fa;se provided cookies are accepted 
